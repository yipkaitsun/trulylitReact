...
class Product extends Component {
  ...
  loadData() {
    ...
    axios.post("https://api.trulylittlethings.com/newproduct-api", {
   
    
    }) .then((response) => {
        this.setState({ activity: response.data.product });
    ...
    return (
      ...
      <CarouselItem responsive={{
        ...
        {this.state.activity.map((arr, index, { length }) => (
          <a key={"product" + index} href={"./product.html?id="+arr.id}>
            <Card style={{ width: '100%' }} className="ActivityCard">
              ...
              <Card.Body className="ActivityCardBody">
                ...
                <Card.Text >
                  <p style={{textAlign:"center"}}>${arr.price}</p>
                </Card.Text>
